...
SELECT pg_catalog.setval(
  'vets_id_seq',
  COALESCE((SELECT MAX(id) + 1 FROM vets) , 1),
  false);
...
SELECT pg_catalog.setval(
  'specialties_id_seq',
  COALESCE((SELECT MAX(id) + 1 FROM specialties) , 1),
  false);
...
SELECT pg_catalog.setval(
  'types_id_seq',
  COALESCE((SELECT MAX(id) + 1 FROM types) , 1),
  false);
...
SELECT pg_catalog.setval(
  'pets_id_seq',
  COALESCE((SELECT MAX(id) + 1 FROM pets) , 1),
  false);
...
SELECT pg_catalog.setval(
  'visits_id_seq',
  COALESCE((SELECT MAX(id) + 1 FROM visits) , 1),
  false);
